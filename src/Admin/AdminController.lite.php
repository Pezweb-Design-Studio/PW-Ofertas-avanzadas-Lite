...
class AdminController
{
	...
	{
		error_log("🔍 [PWOA] getStrategiesByObjective called for: $objective");
		...
		error_log(
			"🔍 [PWOA] Lite classes for $objective: " . count($lite_classes),
		);
		...
		foreach ($lite_classes as $class) {
			if (class_exists($class)) {
				...
				$result[] = $meta;
				error_log("✅ [PWOA] Loaded LITE: " . $meta["name"]);
			}
		}
		...
		error_log(
			"🔍 [PWOA] Pro classes for $objective: " . count($pro_classes),
		);
		...
		foreach ($pro_classes as $class) {
			...
			if ($meta) {
				...
				$result[] = $meta;
				error_log("🔒 [PWOA] Loaded PRO: " . $meta["name"]);
			} else {
				error_log("❌ [PWOA] Pro meta not found for: $class");
			}
		}

		error_log("🔍 [PWOA] Total strategies returned: " . count($result));
		return $result;
	}
