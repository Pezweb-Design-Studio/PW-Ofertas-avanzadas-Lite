...
$behavior_labels = [];
foreach (array_keys($stacking_options) as $behavior_key) {
    $behavior_labels[$behavior_key] = $stacking_options[$behavior_key]['title'] ?? $behavior_key;
}
$behavior_variant = [
    ...
    "content"     => function ($bui) use (
        ...
        <div class="flex justify-between items-center mb-5">
            ...
            <div class="flex items-center gap-2.5">
                <?php $ui->badge([
                    "label"   => sprintf(
                        /* translators: %s: stacking mode label */
                        __('Mode: %s', 'pw-ofertas-avanzadas'),
                ...
                <button id="help-button" type="button"
                    ...
                    title="<?php echo esc_attr(__('Help', 'pw-ofertas-avanzadas')); ?>">?</button>
            </div>
        ...
        <div id="help-modal" class="hidden fixed inset-0 z-[99999] bg-black/60 flex items-center justify-center p-5">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-lg flex flex-col max-h-[85vh]">
                ...
                <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                    <strong class="text-base text-gray-900"><?php echo esc_html(sprintf(
                        /* translators: %s: stacking mode label */
                        __('Active mode: %s', 'pw-ofertas-avanzadas'),
                        $behavior_labels[$behavior] ?? $behavior
                    )); ?></strong>
                ...
                <div class="px-6 py-5 overflow-y-auto flex-1">
                    ...
                    $ui->notice(["type" => $note_type, "message" => wp_kses_post($active_option["note"])]);
                    ?>
                </div>
                <?php endif; ?>

                <div class="flex justify-between items-center px-6 py-4 border-t border-gray-200">
                    <a href="<?php echo esc_url(admin_url("admin.php?page=pwoa-settings")); ?>" class="text-sm text-blue-600 hover:text-blue-800"><?php esc_html_e('Change in Settings', 'pw-ofertas-avanzadas'); ?></a>
                    <button id="close-help-btn" type="button" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 border border-gray-200 rounded text-sm text-gray-700 cursor-pointer"><?php esc_html_e('Close', 'pw-ofertas-avanzadas'); ?></button>
                </div>
